...
import com.example.demo.structures.ListaDoblementeEnlazada;
...
public class ArtistaService {
     ...
     * @throws com.example.demo.exceptions.RecursoNoEncontradoException Si no existe el artista
     */
    public Artista actualizarArtista(String id, ArtistaRegistroDTO artistaDTO) {
        ...
        if (artistaOpt.isEmpty()) {
            throw new com.example.demo.exceptions.RecursoNoEncontradoException("No se encontró el artista con ID: " + id);
        }
    ...
    public void eliminarArtista(String id) {
        if (id == null || id.trim().isEmpty()) {
            throw new com.example.demo.exceptions.DatosInvalidosException("El ID del artista no puede estar vacío");
        }
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(archivo.getInputStream()))) {
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                try {
                    ...
                    try {
                        artista.setGeneroPrincipal(com.example.demo.model.GENERO.valueOf(campos[2].trim().toUpperCase()));
                    ...
                    if (!campos[3].trim().isEmpty()) {
                        try {
                            artista.setGeneroSecundario(com.example.demo.model.GENERO.valueOf(campos[3].trim().toUpperCase()));
                        } catch (IllegalArgumentException e) {
